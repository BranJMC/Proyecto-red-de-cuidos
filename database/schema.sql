...
CREATE TYPE frontend_report_status AS ENUM ('pending', 'in_transit', 'resolved');
CREATE TYPE notification_type AS ENUM ('booking', 'chat', 'verification', 'payment', 'system', 'alert', 'report');
CREATE TYPE job_status AS ENUM ('queued', 'processing', 'completed', 'failed');
CREATE TYPE search_demand_level AS ENUM ('high', 'medium', 'emerging');
...
CREATE TABLE caregiver_profiles (
  ...
  urgent_availability boolean NOT NULL DEFAULT false,
  available_now boolean NOT NULL DEFAULT false,
  available_today boolean NOT NULL DEFAULT false,
  night_shift_available boolean NOT NULL DEFAULT false,
...
CREATE TABLE service_zones (
  ...
  name text NOT NULL,
  demand_level search_demand_level NOT NULL DEFAULT 'medium',
  active_caregivers int NOT NULL DEFAULT 0 CHECK (active_caregivers >= 0),
...
CREATE TABLE caregiver_certifications (
  ...
  expires_at date,
  created_at timestamptz NOT NULL DEFAULT now(),
  UNIQUE (caregiver_id, certification_name)
);

CREATE TABLE caregiver_availability_labels (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  caregiver_id uuid NOT NULL REFERENCES caregiver_profiles(user_id) ON DELETE CASCADE,
  label text NOT NULL,
...
ALTER TABLE caregiver_certifications
...
CREATE TABLE payment_history (
  ...
  status text NOT NULL DEFAULT 'paid' CHECK (status IN ('paid', 'pending-review', 'rejected', 'refunded')),
...
CREATE TABLE reports (
  ...
  booking_id uuid REFERENCES bookings(id) ON DELETE SET NULL,
  caregiver_id uuid REFERENCES caregiver_profiles(user_id) ON DELETE SET NULL,
  report_type text NOT NULL DEFAULT 'servicio',
  ...
  status text NOT NULL DEFAULT 'pending' CHECK (status IN ('pending', 'in_transit', 'resolved', 'open', 'investigating', 'closed')),
  public_status frontend_report_status NOT NULL DEFAULT 'pending',
...
CREATE TABLE dashboard_metrics (
  ...
  accent text NOT NULL CHECK (accent IN ('emerald', 'sky', 'amber', 'rose')),
  sort_order int NOT NULL DEFAULT 0,
  active boolean NOT NULL DEFAULT true,
  updated_at timestamptz NOT NULL DEFAULT now(),
  UNIQUE (role, label)
...
CREATE TABLE analytics_series_points (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  ...
  sort_order int NOT NULL DEFAULT 0,
  created_at timestamptz NOT NULL DEFAULT now(),
  UNIQUE (series_name, point_label)
);

CREATE TABLE caregiver_search_events (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  client_id uuid REFERENCES client_profiles(user_id) ON DELETE SET NULL,
  filters jsonb NOT NULL DEFAULT '{}'::jsonb,
...
CREATE TRIGGER trg_pricing_plans_updated_at BEFORE UPDATE ON pricing_plans FOR EACH ROW EXECUTE FUNCTION set_updated_at();
...
CREATE INDEX idx_caregivers_search_flags ON caregiver_profiles(can_work, available_now, available_today, night_shift_available, weekend_available, emergency_service);
CREATE INDEX idx_caregiver_calendar_date ON caregiver_calendar_blocks(caregiver_id, block_date);
CREATE INDEX idx_caregiver_certifications_caregiver ON caregiver_certifications(caregiver_id);
...
CREATE INDEX idx_payment_history_client_paid ON payment_history(client_id, paid_at DESC);
...
CREATE INDEX idx_dashboard_metrics_role ON dashboard_metrics(role, sort_order);
CREATE INDEX idx_analytics_series ON analytics_series_points(series_name, sort_order);
...
CREATE OR REPLACE VIEW v_frontend_caregivers AS
SELECT
  cp.user_id AS id,
  cp.public_slug AS slug,
  u.full_name AS name,
  cp.headline AS specialty,
  p.name AS province,
  c.name AS city,
  n.name AS neighborhood,
  COALESCE(array_agg(DISTINCT sz.name) FILTER (WHERE sz.id IS NOT NULL), ARRAY[]::text[]) AS zones,
  cp.languages,
  cp.base_hourly_rate AS price_per_hour,
  cp.night_shift_fee,
  cp.weekend_fee,
  cp.emergency_fee,
  cp.rating_average AS rating,
  cp.rating_count AS reviews,
  cp.verified_status = 'approved' AS verified,
  cp.urgent_availability,
  cp.available_now,
  cp.available_today,
  cp.night_shift_available,
  cp.weekend_available,
  cp.emergency_service,
  cp.years_experience AS experience_years,
  COALESCE(array_agg(DISTINCT cs.service_type) FILTER (WHERE cs.id IS NOT NULL), ARRAY[]::text[]) AS service_types,
  cp.bio AS about,
  COALESCE(array_agg(DISTINCT cc.certification_name) FILTER (WHERE cc.id IS NOT NULL), ARRAY[]::text[]) AS certifications,
  COALESCE(array_agg(DISTINCT cal.label) FILTER (WHERE cal.id IS NOT NULL), ARRAY[]::text[]) AS availability,
  cp.service_count,
  cp.rank,
  COALESCE(cp.profile_image_url, u.avatar_url) AS image
FROM caregiver_profiles cp
JOIN users u ON u.id = cp.user_id
LEFT JOIN caregiver_service_zones csz ON csz.caregiver_id = cp.user_id
...
LEFT JOIN caregiver_services cs ON cs.caregiver_id = cp.user_id AND cs.active
LEFT JOIN caregiver_certifications cc ON cc.caregiver_id = cp.user_id
LEFT JOIN caregiver_availability_labels cal ON cal.caregiver_id = cp.user_id
GROUP BY cp.user_id, u.id, p.name, c.name, n.name;

CREATE OR REPLACE VIEW v_frontend_service_zones AS
SELECT
  sz.id,
  COALESCE(p.name, '') AS province,
  COALESCE(c.name, '') AS city,
  COALESCE(n.name, sz.name) AS neighborhood,
  sz.demand_level,
  sz.active_caregivers
...
CREATE OR REPLACE VIEW v_frontend_notifications AS
SELECT
  id,
  user_id,
  title,
...
CREATE OR REPLACE VIEW v_frontend_reports AS
SELECT
  id,
  subject,
  report_type AS type,
  ...
  priority,
  CASE
    WHEN public_status = 'pending' THEN 'pendiente'
    WHEN public_status = 'in_transit' THEN 'en transito'
    ELSE 'resuelto'
  END AS status_label,
  public_status,
  created_at,
