...
import type { Caregiver, CaregiverReminder, DashboardMetric } from '../../types'
import { bookingOperationalTone, serviceLabel } from '../../utils/helpers'
        ...
        <article className="rounded-[32px] border border-slate-200 bg-white/85 p-6 dark:border-white/10 dark:bg-slate-900/70">
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Toca un servicio para abrir el chat y registrar entrada o salida.</p>
            </div>
          ...
          <div className="mt-5 space-y-3">
            {reminders.length ? reminders.map((reminder) => (
              <Link
                key={reminder.id}
                to={reminder.conversationId ? `/caregiver/messages?thread=${reminder.conversationId}` : '/caregiver/messages'}
                className="block rounded-[24px] border border-cyan-100 bg-[linear-gradient(135deg,_rgba(34,211,238,0.14),_rgba(255,255,255,0.88))] px-4 py-4 shadow-sm transition hover:-translate-y-0.5 dark:border-cyan-400/10 dark:bg-[linear-gradient(135deg,_rgba(34,211,238,0.12),_rgba(15,23,42,0.88))]"
              >
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="font-medium text-slate-950 dark:text-white">{serviceLabel(reminder.service)}</p>
                    <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">{reminder.date} | {reminder.time} | {reminder.client}</p>
                  </div>
                  <span className={`rounded-full px-3 py-1 text-xs font-semibold ${bookingOperationalTone(reminder.statusLabel || reminder.status)}`}>
                    {reminder.statusLabel ?? 'Servicio proximo'}
                  </span>
                </div>
                <div className="mt-3 inline-flex items-center gap-2 text-sm font-medium text-cyan-700 dark:text-cyan-300">
                  Abrir seguimiento <ArrowRight className="size-4" />
                </div>
              </Link>
            )) : (
              <div className="rounded-2xl bg-slate-50 px-4 py-6 text-sm text-slate-500 dark:bg-slate-800/60 dark:text-slate-400">
