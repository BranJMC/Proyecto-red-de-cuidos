...
export function MonthAvailabilityCalendar({
  ...
  return (
    <section className="overflow-visible rounded-[32px] border border-slate-200 bg-white/90 p-6 shadow-lg shadow-slate-200/30 dark:border-white/10 dark:bg-slate-900/75 dark:shadow-none">
      <div className="mb-5">
      ...
      <div className={children ? 'grid items-start gap-6 xl:grid-cols-[minmax(0,1fr)_380px]' : ''}>
        <div>
            ...
            {monthCells.map((day, index) =>
              day ? (
                <button
                  ...
                  className={`group relative min-h-18 rounded-[16px] border p-2 text-left transition hover:z-50 hover:-translate-y-0.5 ${
                    selectedDate === day.date ? 'ring-2 ring-cyan-400 ring-offset-2 ring-offset-white dark:ring-offset-slate-900' : ''
                ...
                >
                  ...
                  <div className="pointer-events-none absolute left-1/2 top-full z-[80] hidden w-44 -translate-x-1/2 rounded-2xl border border-slate-200 bg-white/95 p-3 text-left shadow-2xl shadow-slate-200/60 group-hover:block dark:border-white/10 dark:bg-slate-950/95 dark:shadow-none">
                ...
                <div
                  key={`empty-${index}`}
                  className="min-h-18 rounded-[16px] border border-dashed border-slate-200/70 bg-transparent dark:border-white/5"
                />
              ),
            )}
          </div>
